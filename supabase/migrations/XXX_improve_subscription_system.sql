...
-- 1. Đảm bảo unique constraint trên profile_id
DO $$
BEGIN
  IF NOT EXISTS (
...
-- 5. Tạo trigger để auto-expire subscriptions (optional - can be run via cron)
-- This would be called by a scheduled job

-- 6. Tạo index để tối ưu queries
...
-- 7. Tạo function để get subscription với plan details
...
BEGIN
  ...
  GROUP BY s.id, s.profile_id, s.plan, sp.display_name, s.plan_id, s.status, s.start_date, s.end_date, s.billing_cycle, s.payment_method
  ORDER BY s.created_at DESC
  LIMIT 1;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- 8. Cải thiện ensure_seller_has_subscription với better error handling
...
BEGIN
  ...
  ELSIF existing_subscription IS NOT NULL THEN
    UPDATE public.subscriptions
    SET 
      ...
      updated_at = now()
    WHERE profile_id = user_profile_id
      AND (plan IS NULL OR plan_id IS NULL OR status = 'expired');
  END IF;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- 9. Tạo bảng subscription_errors để track errors
...
-- 10. Tạo function để log subscription errors
